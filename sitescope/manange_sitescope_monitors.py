...
def check_monitor(host: str, group_name: str):
    ...
    response = requests.get(find_url, params=params, verify=False, auth=HTTPBasicAuth('administrator', SITESCOPE_PASSWORD))
    ...
    for k, group_name in enumerate(groups_dict):
        if groups_dict[group_name] == 'Group' and host in group_name:
            ...
            if mon_state == "check":
                ...
                break
            response = requests.post(set_url, data=params, verify=False, auth=HTTPBasicAuth('administrator', SITESCOPE_PASSWORD))
